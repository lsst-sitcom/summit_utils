...
def sortSourcesByFlux(
    sources: List[summitUtils.astrometry.fastStarTrackerAnalysis], reverse: bool = False
) -> List[summitUtils.utils.astrometry.fastStarTrackerAnalysis.Source]:
...
) -> List[summitUtils.astrometry.fastStarTrackerAnalysis.Source]:
...
def checkResultConsistency(
    results: Dict[int : List[summitUtils.astrometry.fastStarTrackerAnalysis.Source]],
...
def bboxToMatplotlibRectanle(bbox: geom.Box2I) -> bool:
...
def plotSourcesOnImage(
    parentFilename: str,
    sources: (
        summitUtils.astrometry.fastStarTrackerAnalysis.Source
        | List[summitUtils.astrometry.fastStarTrackerAnalysis.Source]
    ),
) -> None:
...
def plotSource(source: summitUtils.astrometry.fastStarTrackerAnalysis.Source) -> None:
