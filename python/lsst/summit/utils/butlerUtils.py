...
def _assureDict(dataId):
    ...
    if isinstance(dataId, dict):
        return dataId
    elif hasattr(dataId, 'required'):  # dafButler.dimensions.DataCoordinate
        return dict(dataId.required)
    elif hasattr(dataId, 'dataId'):  # dafButler.dimensions.DimensionRecord
        return dict(dataId.dataId.required)
    elif isinstance(dataId, dafButler.NamedKeyMapping):  # (deprecated) DataCoordinate.full views.
...
def getLatissOnSkyDataIds(butler, skipTypes=('bias', 'dark', 'flat'), checkObject=True, full=True,
    ...
    if full:
        expandedIds = [updateDataIdOrDataCord(butler.registry.expandDataId(dataId, detector=0).mapping)
