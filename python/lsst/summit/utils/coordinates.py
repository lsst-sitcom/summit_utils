...
__all__ = ["convertE1E2", "convertIxxIyyIxy"]
...
def convertE1E2(
    ...
    See https://smtn-019.lsst.io/#on-sky-and-hardware-rotation-angles
    for more details.
    ...
    physical_rotator_angle : `float`
        Camera rotator angle in degrees. Equivalent to the rotator telescope
        position (RotTelPos or rtp).
    ...
    sky_angle : `float` | None
        Optional: Sky angle in degrees. This is the orientation of the +𝑌DVCS
        axis (projected on the sky) measured east of north in the International
        Celestial Reference Frame (ICRF). Also called RotSkyPos or given by the
        boresightRotAngle if the rotType=Sky (which is true for all FBS
        observations and many engineering observations). Only needed if the
        coordinate system includes equatorial or nw.
    ...
    if (("nw" or "equatorial") in [input_coordinate_system, output_coordinate_system]) and (
        sky_angle is None
    ):
        raise ValueError("Sky angle must be provided when using nw or equatorial coordinate systems.")
    ...
    if input_coordinate_system == "altaz":
        if output_coordinate_system == "ocs":
            ...
            _, _, _, e1_out, e2_out = convertIxxIyyIxy(
                Ixx, Iyy, [-1 * xy for xy in Ixy], physical_rotator_angle
            )
            if output_coordinate_system == "ccs":
                return e1_out, e2_out
            else:
