...
class starGuideFinder:
    ...
    """
    def __init__(self, readerObject, detname, camera=None, fwhm=12.0, 
                 snr_threshold=3.0, nstamps_min=10,
                 mag_offset_bias_max=0.1, mag_offset_std_max=0.1):
        """
    ...
    def compute_offsets(self, stars):
        ...
        refs = (
            stars
            .loc[stars['stamp'] == -1,
                ['star_id','xpixel','ypixel','xfp','yfp','alt','az']]
            .rename(columns={
                'xpixel': 'x_ref', 'ypixel': 'y_ref',
                'xfp':    'xfp_ref','yfp':   'yfp_ref',
                'alt':    'alt_ref','az':    'az_ref',
            })
        )
    ...
    def build_motion_dataframe(self) -> pd.DataFrame:
        ...
        ref_catalog : pandas.DataFrame
            Must have columns ['xcentroid','ycentroid','flux','flux_err','snr'].
    ...
    def track_stars(self,
        ...
        if not dfs:
            ...
            cols = ['star_id','stamp','xpixel','ypixel',
                    'flux','flux_err', 'mag_offset','snr',
