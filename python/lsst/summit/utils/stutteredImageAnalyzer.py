...
def run_isr(data_id, do_flat=True, do_bias=True):
...
class StutteredImageAnalyzer():
    ...
    def detect_sources(self, exp, threshold=100, do_plot=True, do_background_subtract=True, sigma=3,
                       min_peak_width=12, min_prominence=2, min_distance=50, min_height=5):
    ...
    def find_mean_y_position(self, exp, sources, n_strips, strip_height, sigma=2, y_threshold=5, do_plot=True):
        """
        ...
        for image_half in range(2):
            ...
            half_section = exp.image.array[image_half*(strip_height*n_strips):(image_half+1) *
                                                         (strip_height*n_strips), :]
            if image_half == 1:
                # flip the direction of the second half of the image, so all numberings are consistent
            ...
            sources = sources.drop(sources.loc[(sources.image_half == image_half)].index[(max_values/min_values < y_threshold)]).reset_index(drop=True)
            ...
            if do_plot:
                ...
                plt.imshow(np.arcsinh(10*exp.image.array[image_half*(strip_height*n_strips):(image_half+1) *
                                                         (strip_height*n_strips), :])/10, cmap="magma",
                           origin="lower")
                plt.plot(sources.x_peak.loc[sources.image_half == image_half], np.ones_like(sources.x_peak[sources.image_half == image_half])*strip_height*n_strips/2, 'bx')
 

        return sources


    def catalog_all_sources(self, exp, sources, n_strips, strip_height, do_plot=True, do_plot_all=False, sigma=2,
                            do_background_subtract=True, max_mom2_iter=400, fwhm=20):
        ...
        for image_half in range(2):
            ...
            for index, source in sources.loc[sources.image_half==image_half].iterrows():
                ...
                for strip in range(n_strips):
                    ...
                    try:
                        moments = galsim.hsm.FindAdaptiveMom(galImage, guess_sig=fwhm/2, hsmparams=new_params)

                        if moments.moments_amp > 0:
                            stuttered_object = StutteredObject(index, image_half, strip, source.x_peak, source.y_peak, source.peak_value, 
                                                                moments.moments_amp, moments.moments_centroid.x + x_min - 1, 
                                                                moments.moments_centroid.y + y_min - 1, 
                                                               moments.moments_centroid.y - 1 + source.y_peak - strip_height/2, 
                                                               moments.moments_sigma, moments.moments_rho4,
                                                                moments)
                        ...
                        if do_plot_all:
                            ...
                            plt.plot(moments.moments_centroid.x - 1, moments.moments_centroid.y - 1, 'rx', markersize=15)
                            plt.show()                                
    ...
    def run(self, exp, data_id=np.nan, do_plot=True, do_plot_all=False, method='sigmaclip', threshold=100, sigma=2,
                            do_background_subtract=True, flux_threshold=3, min_object_number=5, y_threshold=5):     
        ...
        stuttered_image : `dataclass`
            Dataclass with information about the image and the sources in the image.
        """
        strip_height, n_strips, strip_exp_time = get_stutter_properties(exp)
        ...
        sources = self.find_mean_y_position(exp, sources, n_strips, strip_height, sigma=sigma, do_plot=do_plot, y_threshold=y_threshold)
         
        stuttered_object_catalog = self.catalog_all_sources(exp, sources, n_strips, strip_height, do_plot_all=do_plot_all)
        
        filtered_stuttered_object_catalog = self.filter_objects(sources, stuttered_object_catalog, 
        ...
        if do_plot:
            for image_half in range(2):
                ...
                plt.figure(figsize = (10, 20))
                plt.imshow(np.arcsinh(10*half_section)/10, cmap="magma", origin="lower")
                plt.plot([source.fitted_centroid_x for source in filtered_stuttered_object_catalog if source.image_half==image_half],
                         [source.fitted_centroid_y for source in filtered_stuttered_object_catalog if source.image_half==image_half], 'x')
                plt.show() 
                
        return StutteredImage(filtered_stuttered_object_catalog, n_strips, strip_height, strip_exp_time, backgrounds, data_id)

    def filter_objects(self, sources, stuttered_object_catalog, flux_threshold=3, min_object_number=5):
        """ Remove all objects that are too dim relative to the other objects, and all sources where there
        are too few 
        ...
        min_object_number : `integer`
            The minimum number of objects for which it's worth doing calculations
        ...
        for index in sources.index:
            # Filter out sources with flux outside threshold standard deviations.
            fluxes = [object.fitted_flux for object in stuttered_object_catalog 
                                if object.source_number == index]
            ...
            if len(fluxes) > 0:
                ...
                # add objects to the filtered catalog only if their flux is within threshold or it's the first object in the strip and too bright
                source_objects = [object for object in stuttered_object_catalog 
                                  if (((object.source_number == index) and 
                                      (np.abs(object.fitted_flux - mean_flux) <= flux_threshold * std_flux)) or
                                 ((object.fitted_flux == fluxes[-1]) and (fluxes[-1] > mean_flux)))]

                # convert the strips in this list to stutter number, where 0 is the final stutter, or the one with the highest strip number
        ...
        # flatten list
        filtered_stuttered_object_catalog = [object for source_list in filtered_stuttered_object_catalog for object in source_list]    
